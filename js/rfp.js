$('#nombreModulo').text('Request for Proposal');
$('#nombreModuloM').text('Request for Proposal');
...
$(document).ready(function () {
		...
		$.get('../controllers/Load.php', {
			...
			if (resp) {
				...
				$.each(respuesta.Results, function (index, dato) {
					if (dato.VariableId == 29) {
						$('#viYear').val(dato.Value);
					};
					if (dato.VariableId == 5) {
						$('#viType').val(dato.Value);
					};
					if (dato.VariableId == 26) {
						$('#viMake').val(dato.Value);
					};
				});
			}
			modalHide('spinner');
	...
	$(document).on('click', '#addPCI', function () {
		$yearFrom = $('#pciYearFrom').val();
		$yearTo = $('#pciYearTo').val();
		$company = $('#pciCompany').val();
		$losses = $('#pciLosses').is(':checked') ? 'Yes' : 'NO';
		$details = $('#pciDetails').val();
		$driverInvolved = $('#pciDriver').val();
		$row = `<tr class='text-start'><td>${$yearFrom}</td><td>${$yearTo}</td><td>${$company}</td><td>${$losses}</td><td>${$details}</td><td>${$driverInvolved}</td><td class='text-end'><div class="btn btn-outline-danger fw-bold removeRow"><i class="fa-solid fa-square-minus fa-lg"></i> Remove</div></tr>`;
	...
	$(document).on('click', '#primaryLiabilityBtn', function () {
		$('#primaryLiabilityBtn').addClass('active');
		$('#nonTruckingLiabilityBtn').removeClass('active');
		$('#primaryLiabilityGrp').removeClass('d-none');
		$('#cargoGrp').removeClass('d-none');
	});

	$(document).on('click', '#nonTruckingLiabilityBtn', function () {
		$('#primaryLiabilityBtn').removeClass('active');
		$('#nonTruckingLiabilityBtn').addClass('active');
		$('#primaryLiabilityGrp').addClass('d-none');
		$('#primaryLiabilityGrp input').val('');
		$('#cargoGrp').addClass('d-none');
	});
});
