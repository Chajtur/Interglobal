...
$data = $daysOff->getDaysOff(getUser());
$dataDate = 0;
    ...
    <div id="" class="accordion-header bg-sky-950 justify-between" data-content='vacacionesContent'>
        ...
        <div class="flex">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6 arrowClose">
                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" />
            </svg>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6 arrowOpen">
...
<div id="vacacionesContent" class="accordion-content h-96">
    <div class="tab-group flex flex-row mt-2" id="tabVacaciones">
        ...
        {
            $dataDate = $daysOff->getApprovedDaysOffbyDay($fecha); ?>
            <div class="text-tab active" id="tabVacacionesPersonal" data-target="VacacionesPersonal">Staff Requested Vacations</div>
        <?php }
        ; ?>
    </div>
    <!-- Tab panes -->
    <div class="tab-content">
        <div class="tab-pane h-80 active" id="contentVacacionesPersonal">
            <?php if ($dataDate)
            { ?>
                    ...
                    <div class="text-end row-group overflow-y-auto h-64">
                        <?php foreach ($dataDate as $key => $vacacion)
                        {
                            ...
                            <div data-target='#id<?php echo $vacacion["id"] ?>'
                                class='text-white row-tab cursor-pointer flex flex-row p-2 <?php echo $bgColor ?>'>
                                ...
                                <div colspan='4'>
                                    ...
                                    { ?>
                                        <div class="flex flex-row w-full justify-end my-1">
                                            <div class="">
                                                <button type="button" class="btn-danger shadow-none solicitarVacaciones"
                                                    data-searchDate='<?php $_POST['fecha'] ?>'>Reject</button>
                                                <button type="button" class="btn-success shadow-none solicitarVacaciones"
                                                    data-searchDate='<?php $_POST['fecha'] ?>'>Approve</button>
                                            </div>
                                        </div>
                                    <?php } ?>
                                </div>
...
<script>
    $(document).ready(function () {
        $('#requestVacationDivWithDate').load('../components/buttons/requestVacationButton.php',
